    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-8">
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 px-4 sm:px-0">
                <h1 class="text-3xl sm:text-4xl font-bold text-indigo-900">{{ __('Дома и доступ жильцов') }}</h1>
                <a href="{{ route('dashboard') }}" wire:navigate class="text-base font-semibold text-indigo-700 hover:text-indigo-900">{{ __('← В кабинет') }}</a>
            </div>
            ...
            <div class="app-card mx-4 sm:mx-0">
                <div class="p-6 space-y-4 border-b border-gray-100">
                    <h2 class="text-2xl font-bold text-indigo-900">{{ __('Новый дом') }}</h2>
                ...
                <div class="p-6 space-y-4 border-b border-gray-100">
                    <h2 class="text-2xl font-bold text-indigo-900">{{ __('Квартиры выбранного дома') }}</h2>
                    @if ($this->buildings->isEmpty())
                        <p class="text-sm text-gray-500">{{ __('Сначала добавьте дом выше.') }}</p>
                    @elseif (! $building_id)
                    ...
                    @else
                        <form wire:submit="createApartment" class="flex flex-wrap gap-4 items-end max-w-xl">
                            ...
                            <x-primary-button type="submit">{{ __('Добавить квартиру') }}</x-primary-button>
                        </form>
                        <p class="text-base text-slate-700">
                            {!! __('Полный список с поиском, статусами показаний, приглашениями и доступом — :link.', [
                                'link' => '<a href="'.route('manager.apartments').'" class="font-medium text-indigo-600 hover:text-indigo-800">'.__('таблица квартир').'</a>',
                            ]) !!}
                        </p>
                    @endif
                </div>
                ...
                <div class="p-6 space-y-4">
                    <h2 class="text-2xl font-bold text-indigo-900">{{ __('Новый жилец') }}</h2>
                    <p class="text-base text-slate-700">{{ __('Вы указываете данные и квартиру; пароль жилец задаёт сам по ссылке из письма (с подтверждением). Если письмо не дошло или пароль забыт — отправьте ссылку снова в разделе «Квартиры».') }}</p>
                    @if ($this->buildings->isEmpty() || ! $building_id)
                        <p class="text-sm text-gray-500">{{ __('Выберите дом во вкладке, затем квартиру из списка этого дома.') }}</p>
                    @else
                        <form wire:submit="createResident" class="grid gap-4 max-w-xl">
                            ...
                            <div>
                                <x-input-label for="raid" :value="__('Квартира в этом доме')" />
                                <select wire:model="resident_apartment_id" id="raid" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">
                                    ...
                                    @foreach ($this->apartmentsForBuilding as $a)
                                        <option value="{{ $a->id }}">{{ __('кв. :number', ['number' => $a->number]) }}</option>
                                    @endforeach
                                </select>
